...
function httpRequestLight({ uri, data = null, method = 'GET', headers = null, json = true, timeout = 8000 }) {
    return new Promise((resolve, reject) => {
        let onResponse = false;
        let options = {};
...
function httpRequest({ uri, data = null, body = null, qs = null, method = 'GET', headers = null, json = true, timeout = 8000 }) {
    return new Promise((resolve, reject) => {
        ...
        req.on('timeout', () => {
            ...
            if (!onResponse) {
                onResponse = true;
                return reject({ error: new Error('Request timed out'), body: null });
            }
        });
        // Set the timeout on the request
        req.setTimeout(timeout);
...
function proxyRequestMonitor(req, res, options, extraOptions = {}) {
    return new Promise((resolve, reject) => {
        ...
        proxyReq.on('response', (proxyRes) => {
            ...
            // Emit statusCode event for caching
            if (extraOptions && extraOptions.events && extraOptions.events.statusCode) {
                extraOptions.events.statusCode(proxyRes.statusCode, proxyRes.headers);
            }
